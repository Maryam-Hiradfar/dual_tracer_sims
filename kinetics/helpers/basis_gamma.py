...
def gamma_shape(t: np.ndarray, t0: float, tau: float, scale: float = 1.0) -> np.ndarray:
    x = np.maximum(t - t0, 0.0)
    return scale * (x / tau) * np.exp(-x / tau)

def build_uniform_gamma_library(t: np.ndarray, config: GammaBasisConfig):
    config.validate()

    t0_max = float(t[-1]) - config.t0_max_offset
